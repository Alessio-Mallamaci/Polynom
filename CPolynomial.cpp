...
#include "CPolynomial.h"  // Inclusione del file header

/// @brief default constructor
//Implementazione costruttore di deault
...
Polynomial::~Polynomial() {
	...
	// Libera memoria dinamica per evitare errore di "memory leak"
	if (coeff != NULL) {
		delete coeff;
		coeff = NULL;
	}
...
/// @brief overload operator +
//  Implementazione operatore di somma tra due oggetti (+) --> Restituisce un nuovo oggetto Polynomial che rappresenta il polinomio risultante dalla somma dei due polinomi.
Polynomial Polynomial::operator+(const Polynomial& p) {
    
	int i;										//Inizializzo variabile contatore utilizzata per il ciclo for
	
	Polynomial newP; 							// Dichiara un nuovo oggetto Polynomial, che conterrà la somma finale.
	int newSize = max(degree, p.degree) + 1; 	//Il grado massimo tra i due polinomi può determinare la lunghezza del polinomio risultato.
	double* newCoeff = new double[newSize]; 	//Alloca dinamicamente un array newCoeff per contenere i coefficienti della somma.
	...
	//Inizializza tutti gli elementi dell’array newCoeff a 0
	for (i = 0; i < newSize; ++i) 
        newCoeff[i] = 0.;
    
	//Copia i coefficienti dell’oggetto corrente
    for (i = 0; i <= degree; ++i) 
        newCoeff[i] += coeff[i];
    
	// Somma i coefficienti del polinomio p
    for (i = 0; i <= p.degree; ++i) 
        newCoeff[i] += p.coeff[i];
    
	// Richiamo il costruttore parametrico per inizializzare newP con i coefficienti calcolati.
    newP= Polynomial(newCoeff, newSize);
	
	delete newCoeff; // Deallocazione della memoria dinamica --> chat gpt suggerisce di scrivere "delete[] newCoeff;" perchè usare delete su un array causa comportamento indefinito, quindi un bug.
    
    return newP; // Restituisce il nuovo oggetto Polynomial
}
